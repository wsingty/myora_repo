...
xserials = []         # list wills tore infiniband serial numbers from test events (unique ones)
for i in test:
  if "INFINI" in i:
    i = i.split()
                    ...
                    if serial in eachline:                              # check is serial is in the lines of con_list3
                        ...
                        for line in sid_info2:
                            if thesid in line:
                                ...
                                #l0 = l1+"   "+l2+"   "+val+"   "+l5+"   "+l6
                                l0 = l1+"   "+hostnames+"   "+val+"   "+l5
                                if l0 not in TDlist:
                                  if "INFINIBAND" not in l0:
                                    TDlist.append(l0)
                                  if "INFINIBAND" in l0:
                                      for eachline in newcon_col:
                                        if serial in eachline:
                                          if "ILOM" in eachline:
                                            line = eachline.split()
                                            line = line[1]
                                            hostname = line.strip()
...
for i in tdlist3:                          # append lines that have infiniband and make necessary changes
    if "INFINIBAND" in i:
...
#valvalues = "SerialNo Product-Description\tILOM-hostname\tOS-hostname\tILOM\tOS\tStatus\tASRM-Hostname"
valvalues = "SerialNo Product-Description\tILOM-hostname\tOS-hostname\tILOM\tOS\tStatus"
